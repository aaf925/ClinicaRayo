...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($numero_tarjeta) || empty($mes_vencimiento) || empty($cvc) || empty($titular_tarjeta)) {
        echo "<script>alert('Por favor, complete todos los campos.'); window.location.href='continuarPagarCarrito.php';</script>";
        exit();
    }

    // Validar formato del número de tarjeta (ejemplo básico)
    if (!preg_match('/^\d{16}$/', $numero_tarjeta)) {
        echo "<script>alert('Número de tarjeta no válido.'); window.location.href='continuarPagarCarrito.php';</script>";
        exit();
    }

    // Validar CVC (3 dígitos)
    if (!preg_match('/^\d{3}$/', $cvc)) {
        echo "<script>alert('CVC no válido.'); window.location.href='continuarPagarCarrito.php';</script>";
        exit();
    }

    // Validar mes de vencimiento (formato MM/YY)
    if (!preg_match('/^(0[1-9]|1[0-2])\/\d{2}$/', $mes_vencimiento)) {
        echo "<script>alert('Fecha de vencimiento no válida.'); window.location.href='continuarPagarCarrito.php';</script>";
        exit();
    }
    ...
    if (!$pedido) {
        echo "<script>alert('No se encontró un pedido para procesar el pago.'); window.location.href='carrito.php';</script>";
        exit();
    }
    ...
    if ($pago_exitoso) {
        ...
        $stmt_actualizar = $conexion->prepare($sql_actualizar_pedido);
        $stmt_actualizar->bind_param("i", $id_pedido);
        $stmt_actualizar->execute();

        if ($stmt_actualizar->affected_rows > 0) {
            echo "<script>alert('Pago completado con éxito.'); window.location.href='confirmacionPago.php';</script>";
        } else {
            echo "<script>alert('Error al actualizar el estado del pedido.'); window.location.href='continuarPagarCarrito.php';</script>";
        }
    } else {
        echo "<script>alert('El pago no se pudo completar. Intente nuevamente.'); window.location.href='continuarPagarCarrito.php';</script>";
    }
} else {
    echo "<script>alert('Acceso no autorizado.'); window.location.href='carrito.php';</script>";
    exit();
}
